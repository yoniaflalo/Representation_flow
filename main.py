...
im1 = np.array(Image.open("eval-data/Basketball/frame07.png").convert('L'))
im2 = np.array(Image.open("eval-data/Basketball/frame09.png").convert('L'))
x1=torch.tensor(im1)
x2=torch.tensor(im2)
x=torch.stack([x1,x2]).unsqueeze(0).float()
t = TVL1OF(size_in = im1.shape, num_iter=50, lambda_=1e-1, tau=1/16, theta=0.001)
a = t(x)
tv=a.data.cpu().numpy()
tv1=tv[:,0,:,:].squeeze()
tv2=tv[:,1,:,:].squeeze()
...
#
# im1=cv2.imread("eval-data/Basketball/frame07.png")
# im2=cv2.imread("eval-data/Basketball/frame09.png")
#
